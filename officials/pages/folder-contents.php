<?php
  ...
  $get_folder_contents = $conn->prepare("SELECT * FROM officials_folder_tbl WHERE folder_id = :folder_id LIMIT 1");
  ...
  if ($get_folder_contents->rowCount() === 1) {
    ...
    if($folder_data->$officials_folder_permission === "No") {
    ...
    else if($folder_data->is_archive === "Yes") {
      $_SESSION["query-status"] = [
        "status" => "danger",
        "message" => "Folder not found or archived!"
      ];
            ...
            <div class="table-responsive">
              <table class="table align-middle table-hover shadow-sm rounded datatable">
                ...
                <tbody class="bg-white">
                  ...
                  SELECT 
                    ofl.*, CONCAT(oa.first_name, ' ', oa.last_name) AS 'uploader_name'
                  FROM officials_files_tbl ofl
                  LEFT JOIN officials_accounts_tbl oa
                  ON ofl.uploaded_by = oa.official_id
                  WHERE folder_id = :folder_id AND ofl.is_archive = :is_archive
            ...
            ");
                  ...
                  while ($file_data = $get_folder_files->fetch()) { ?>
                    <tr class="text-center border-bottom">
                      ...
                      <td class="fw-bold">
                        <?php 
                          if($file_data["uploaded_by"] == $official_id) {
                            echo "You";
                          }

                          else {
                            echo htmlspecialchars($file_data["uploader_name"]); 
                          }
                        ?>
                      </td>
                      ...
                      <td>
                        <div class="d-flex gap-1 justify-content-center">
                          <?php
                            if($file_data[$officials_file_permission] === "Yes") {
                          ?>
